...
`runtime_dsp_object_bound_wav_resync_demo` now writes a demo-local HTML audio report with native audio controls plus render, phase, and WAV artifact status sections. This is the first mouse-and-ears oriented artifact, but it is still generated by an example and is not a sandbox app.

The HTML report remains demo-only and is a sandbox-status proof, not a scheduler, executor, project serialization format, audio engine, production file API, plugin/UI layer, or new DSP ownership model.
